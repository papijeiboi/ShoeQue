...
import kotlinx.android.synthetic.main.activity_success_add_user.*

class SuccessAddUserActivity : AppCompatActivity() {
    private var userType: String = ""

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
        setContentView(R.layout.activity_success_add_user)

        userType = intent.getStringExtra("userType")!!

        btn_success_add_user_done.setOnClickListener {
            when(userType){
                "stock" -> {
                    runOnUiThread {
                        finish()
